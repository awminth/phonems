...
const CashReport: React.FC = () => {
    ...
    const buildQueryString = useCallback(() => {
        ...
        return `${API_ENDPOINTS.REPORT_CASH}?${params.toString()}`;
    }, [page, limit, appliedFilters]);

    // SWR for cash reports
    const { data, error, isLoading, mutate } = useSWR<CashReportResponse>(
        buildQueryString(),
        fetcher,
    ...
    const { data: voucherData, error: voucherError, isLoading: isLoadingVoucher, mutate: mutateVoucher } = useSWR<{success: boolean, data: VoucherDetails}>(
        isPrintModalOpen && printVNO ? API_ENDPOINTS.REPORT_CASH_VOUCHER(printVNO) : null,
        fetcher,
        { revalidateOnFocus: false, dedupingInterval: 5000 }
    );
    ...
    // Export to Excel
    const exportToExcel = () => {
        const title = `Cash Sale Report (${appliedFilters.fromDate || 'All'} to ${appliedFilters.toDate || 'Today'})`;
        const headers = ["Date", "Folio No", "Customer", "IMEI", "SubTotal", "Disc", "Tax", "Other", "Total", "Purchase Price", "Profit", "Cash", "Refund", "Cashier"];
        ...
        exportStyledExcel(title, headers, excelData, `cash_report_${timestamp}.xlsx`, 'Cash Report');
    };
        ...
        <div className="h-screen bg-gray-900 text-white flex flex-col font-sans overflow-hidden">
            ...
            <header className="bg-gray-800 shadow-md p-4 flex items-center border-b border-gray-700 sticky top-0 z-40 shrink-0 h-16">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <h1 className="text-xl font-bold">Cash Report</h1>
                </div>
            </header>
                    ...
                    {error && (
                        <div className="flex-1 flex items-center justify-center">
                            <div className="bg-red-900/30 border border-red-700 rounded-xl p-6 text-center">
                                <p className="text-red-400">Failed to load cash reports.</p>
                                ...
                                <div className="overflow-x-auto">
                                    ...
                                    <thead>
                                        <tr className="bg-gray-900/50 text-gray-400 text-xs uppercase font-bold tracking-wider border-b border-gray-700">
                                            ...
                                            <th className="p-4 text-right whitespace-nowrap">Refund</th>
                        ...
                        {isLoadingVoucher ? (
                            ...
                            <>
                                <div className="overflow-y-auto flex-1 min-h-0">
                                    <Voucher
                                        voucher={{
                                            ...
                                            paymentType: voucherDetails.voucher.paymentType || 'Cash',
                                ...
                                <div className="p-4 bg-gray-50 border-t flex gap-3 print:hidden">
                                    ...
                                    <button 
                                        onClick={handlePrint}
                                        className="flex-1 px-4 py-2 bg-emerald-600 text-white rounded hover:bg-emerald-700 text-sm font-medium flex items-center justify-center gap-2 shadow-sm"
                                    >
