...
const ProductCategoryReport: React.FC = () => {
    ...
    const [activeType, setActiveType] = useState<'phone' | 'accessory' | 'service' | 'spare'>('phone');
    ...
    const handleTypeChange = (type: 'phone' | 'accessory' | 'service' | 'spare') => {
        setActiveType(type);
        setPage(1);
    };
    ...
    const handleExport = () => {
        ...
        const typeLabel = activeType === 'phone' ? 'Phones' : activeType === 'accessory' ? 'Accessories' : activeType === 'service' ? 'Services' : 'Spare Parts';
        const title = `${typeLabel} Sale Report`;
        
        const headers = activeType === 'phone'
            ? ['Date', 'VNO', 'Item Name', 'Specification', 'Code', 'Branch', 'Customer', 'Qty', 'Price', 'Total', 'Purchase Price', 'Profit']
            : ['Date', 'VNO', 'Item Name', 'Code', 'Branch', 'Customer', 'Qty', 'Price', 'Total', 'Purchase Price', 'Profit'];
        
        const excelData = sales.map((s: any) => activeType === 'phone' ? [
        ...
        excelData.push(activeType === 'phone'
            ? ['', '', '', '', '', '', 'TOTAL', totals.qty, '', totals.amount, totals.purchasePrice || 0, totals.profit || 0]
            : ['', '', '', '', '', 'TOTAL', totals.qty, '', totals.amount, totals.purchasePrice || 0, totals.profit || 0]
        );

        exportStyledExcel(title, headers, excelData, `${activeType}_sale_report_${new Date().toISOString().split('T')[0]}.xlsx`, typeLabel);
    };
        ...
        <div className="h-screen bg-gray-900 text-white flex flex-col font-sans overflow-hidden">
            <header className="bg-gray-800 shadow-md p-4 flex items-center justify-between border-b border-gray-700 h-16 shrink-0">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-xl font-bold">Phones & Accessories & Services</h1>
                </div>
            ...
            {/* Tabs */}
            <div className="bg-gray-800 px-4 border-b border-gray-700 flex gap-2 shrink-0">
                <button 
                    onClick={() => handleTypeChange('phone')}
                    className={`px-6 py-3 text-sm font-bold border-b-2 transition-all flex items-center gap-2 ${activeType === 'phone' ? 'border-blue-500 text-blue-400 bg-blue-500/5' : 'border-transparent text-gray-500 hover:text-gray-300'}`}
                >
                    <Smartphone size={18} /> Phones
                </button>
                <button 
                    onClick={() => handleTypeChange('accessory')}
                    className={`px-6 py-3 text-sm font-bold border-b-2 transition-all flex items-center gap-2 ${activeType === 'accessory' ? 'border-emerald-500 text-emerald-400 bg-emerald-500/5' : 'border-transparent text-gray-500 hover:text-gray-300'}`}
                >
                    <Package size={18} /> Accessories
                </button>
                <button 
                    onClick={() => handleTypeChange('spare')}
                    className={`px-6 py-3 text-sm font-bold border-b-2 transition-all flex items-center gap-2 ${activeType === 'spare' ? 'border-amber-500 text-amber-400 bg-amber-500/5' : 'border-transparent text-gray-500 hover:text-gray-300'}`}
                >
                    <Cpu size={18} /> Spare Parts
                </button>
                <button 
                    onClick={() => handleTypeChange('service')}
                    className={`px-6 py-3 text-sm font-bold border-b-2 transition-all flex items-center gap-2 ${activeType === 'service' ? 'border-purple-500 text-purple-400 bg-purple-500/5' : 'border-transparent text-gray-500 hover:text-gray-300'}`}
                >
                    ...
                    <div className="flex-1 overflow-auto bg-gray-800 rounded-xl border border-gray-700 shadow-lg">
                        {isLoading ? (
                            ...
                            <table className="w-full text-left border-collapse min-w-[1000px]">
                                <thead className="bg-gray-900/50 sticky top-0 z-10 text-[10px] uppercase font-bold tracking-wider text-gray-400 border-b border-gray-700">
                                    <tr>
                                        <th className="p-4 text-center w-16">#</th>
                                        <th className="p-4">Date / VNO</th>
                                        <th className="p-4">Item Details</th>
                                        {activeType === 'phone' && <th className="p-4">Specification</th>}
                                ...
                                <tbody className="divide-y divide-gray-700">
                                    {sales.map((item: any, index: number) => (
                                        <tr key={item.id} className="hover:bg-gray-750 transition-colors">
                                            ...
                                            {activeType === 'phone' && <td className="p-4 text-sm text-gray-300 font-medium">{item.specification || '-'}</td>}
                                            <td className="p-4">
                                                <div className="flex items-center gap-1 text-emerald-400 text-xs">
                                                    <Building2 size={12} /> {item.branchName || '-'}
                                                </div>
                                            </td>
                                            <td className="p-4 text-sm text-blue-300 font-medium">
                                                {item.customerName || '-'}
                                            </td>
                                ...
                                <tfoot className="bg-gray-900/30 font-bold border-t border-gray-700">
                                    <tr className="border-b border-gray-700/50 text-xs">
                                        <td colSpan={activeType === 'phone' ? 6 : 5} className="p-4 text-right text-gray-400 uppercase">Total Page {page}</td>
                                    ...
                                    <tr className="bg-gray-900/60 text-xs">
                                        <td colSpan={activeType === 'phone' ? 6 : 5} className="p-4 text-right text-gray-400 uppercase">Grand Total</td>
