...
const ReceiveReport: React.FC = () => {
    ...
    const buildQuery = () => {
        ...
        params.append('status', 'Received'); // Strictly received items for this report
        ...
        if (isAdmin) {
            ...
            params.append('toBranchId', userBranchId);
        }
    ...
    const handleExport = () => {
        ...
        const title = 'Inventory Receive Report';
        const headers = ['Date Received', 'From Branch', 'To Branch', 'Items', 'Receiver', 'Remark'];
        const excelData = transfers.map((t: any) => [
            new Date(t.receivedDate || t.transferDate).toLocaleDateString(),
            t.fromBranchName,
            t.toBranchName,
            `${t.itemCount} items`,
            t.receiverName || '-',
        ...
        exportStyledExcel(title, headers, excelData, `receive_report_${new Date().toISOString().split('T')[0]}.xlsx`, 'Receive Report');
    };
        ...
        <div className="h-screen bg-gray-900 text-white flex flex-col font-sans overflow-hidden">
            <header className="bg-gray-800 shadow-md p-4 flex items-center justify-between border-b border-gray-700 h-16 shrink-0">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-xl font-bold">Receive Report</h1>
                </div>
            </header>
            ...
            <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
                <aside className="w-full lg:w-80 bg-gray-800 border-b lg:border-b-0 lg:border-r border-gray-700 p-4 flex flex-col gap-4 overflow-y-auto shrink-0 shadow-xl">
                    <div className="flex items-center gap-2 text-purple-400 border-b border-gray-700 pb-2">
                        <Filter size={20} />
                    ...
                    <div className="space-y-4">
                        {isAdmin && (
                            <>
                                <div>
                                    <label className="text-sm font-medium text-gray-400 block mb-1">Target Branch (To)</label>
                                    <select 
                                        value={toBranchId}
                                ...
                                <div>
                                    <label className="text-sm font-medium text-gray-400 block mb-1">Sender Branch (From)</label>
                                    <select 
                                        value={fromBranchId}
                                    ...
                                    {transfers.map((item: any) => (
                                        <tr key={item.id} className="hover:bg-gray-750 transition-colors">
                                            <td className="p-4 text-sm text-gray-300 whitespace-nowrap">
                                                {new Date(item.receivedDate || item.transferDate).toLocaleDateString()}
                                            </td>
                                            <td className="p-4">
                                                <div className="flex items-center gap-2 text-sm text-white">
                                                    <span>{item.fromBranchName}</span>
                                                    <ArrowRightLeft size={12} className="text-gray-600" />
                                                    <span className={item.toBranchId == userBranchId ? "text-emerald-400 font-bold" : ""}>{item.toBranchName}</span>
                                                </div>
                                            </td>
                                            ...
                                            <td className="p-4 text-center">
                                                <span className="px-2 py-1 text-[10px] rounded-full border bg-emerald-900/30 text-emerald-400 border-emerald-800/50">
                                                    {item.status}
                                                </span>
                                            </td>
                                            <td className="p-4 text-sm text-gray-500">{item.receiverName || '-'}</td>
                                            <td className="p-4 text-center">
                                                <button 
                    ...
                    <div className="bg-gray-800 rounded-3xl w-full max-w-3xl max-h-[90vh] shadow-2xl border border-gray-700 flex flex-col overflow-hidden">
                        <div className="p-6 border-b border-gray-700 flex justify-between items-center bg-gray-800/80 sticky top-0">
                            <h2 className="text-xl font-bold text-white flex items-center gap-2">
                                <FileText className="text-purple-400" size={24} /> Receive Details
                            </h2>
                        ...
                        <div className="p-6 overflow-y-auto space-y-6">
                            <div className="grid grid-cols-2 gap-6 text-sm">
                                ...
                                <div>
                                    <div className="text-gray-500 uppercase font-bold mb-1 text-[10px]">Date Received</div>
                                    <div className="text-white font-bold">{new Date(selectedTransfer.receivedDate || selectedTransfer.transferDate).toLocaleString()}</div>
                                </div>
                                <div>
                                    <div className="text-gray-500 uppercase font-bold mb-1 text-[10px]">Receiver</div>
                                    <div className="text-emerald-400 font-bold">{selectedTransfer.receiverName || '-'}</div>
                                </div>
                            </div>
                                        ...
                                        {selectedTransfer.items?.map((item, idx) => (
                                            <tr key={idx}>
                                                ...
                                                <td className="px-4 py-3 text-center">
                                                    {item.imei ? (
                                                        <code className="text-purple-400 font-mono text-[10px] bg-purple-900/20 px-1.5 py-0.5 rounded">{item.imei}</code>
                                                    ) : (
