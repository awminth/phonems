...
const CreditReport: React.FC = () => {
    ...
    // Pay View Modal
    const [isPayViewModalOpen, setIsPayViewModalOpen] = useState(false);
    const [paymentVNO, setPaymentVNO] = useState<string | null>(null);
    ...
    const buildQueryString = useCallback(() => {
        ...
        return `${API_ENDPOINTS.REPORT_CREDIT}?${params.toString()}`;
    }, [page, limit, appliedFilters]);

    // SWR for credit reports
    const { data, error, isLoading, mutate } = useSWR<CreditReportResponse>(
        buildQueryString(),
        fetcher,
    ...
    // SWR for payment details (only fetch when modal is open and VNO is set)
    const { data: paymentData, error: paymentError, isLoading: isLoadingPayments, mutate: mutatePayments } = useSWR<{success: boolean, data: any}>(
        isPayViewModalOpen && paymentVNO ? API_ENDPOINTS.REPORT_CREDIT_PAYMENTS(paymentVNO) : null,
        fetcher,
        { revalidateOnFocus: false, dedupingInterval: 5000 }
    );
    ...
    const { data: voucherData, error: voucherError, isLoading: isLoadingVoucher, mutate: mutateVoucher } = useSWR<{success: boolean, data: VoucherDetails}>(
        isPrintModalOpen && printVNO ? API_ENDPOINTS.REPORT_CREDIT_VOUCHER(printVNO) : null,
        fetcher,
        { revalidateOnFocus: false, dedupingInterval: 5000 }
    );
    ...
    const paymentDetails = paymentData?.data || null;
    ...
    // Export to Excel
    const exportToExcel = () => {
        const title = `Credit Sale Report (${appliedFilters.fromDate || 'All'} to ${appliedFilters.toDate || 'Today'})`;
        const headers = ["Date", "Folio No", "Customer", "IMEI", "SubTotal", "Disc", "Tax", "Other", "Total", "Purchase Price", "Profit", "Cash", "Credit", "Cashier"];
        ...
        exportStyledExcel(title, headers, excelData, `credit_report_${timestamp}.xlsx`, 'Credit Report');
    };
    ...
    const handleViewPayments = (vno: string) => {
        setPaymentVNO(vno);
        setIsPayViewModalOpen(true);
    };
    ...
    return (
        <div className="h-screen bg-gray-900 text-white flex flex-col font-sans overflow-hidden">
            ...
            {/* Toast Notification */}
            {notification && (
                <div className={`fixed top-20 right-4 z-[200] px-6 py-3 rounded-lg shadow-lg text-white font-medium flex items-center animate-bounce ${
            ...
            <header className="bg-gray-800 shadow-md p-4 flex items-center border-b border-gray-700 sticky top-0 z-40 shrink-0 h-16">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <h1 className="text-xl font-bold">Credit Report</h1>
                </div>
            </header>
                    ...
                    {error && (
                        <div className="flex-1 flex items-center justify-center">
                            <div className="bg-red-900/30 border border-red-700 rounded-xl p-6 text-center">
                                <p className="text-red-400">Failed to load credit reports.</p>
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="bg-gray-900/50 text-gray-400 text-xs uppercase font-bold tracking-wider border-b border-gray-700">
                                            ...
                                            <th className="p-4 whitespace-nowrap">IMEI</th>
                                            ...
                                            <th className="p-4 text-right whitespace-nowrap">Cash</th>
                                            <th className="p-4 text-right whitespace-nowrap">Credit</th>
                                            <th className="p-4 whitespace-nowrap">Cashier</th>
                                            <th className="p-4 text-center min-w-[120px] whitespace-nowrap">Action</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-700">
                                        {reports.length > 0 ? (
                                            <>
                                                {reports.map((item, index) => (
                                                    <tr key={item.id} className="hover:bg-gray-750 transition-colors">
                                                        ...
                                                        <td className={`p-4 text-sm text-right font-bold ${(item.profit || 0) >= 0 ? 'text-blue-400' : 'text-red-400'}`}>{(item.profit || 0).toLocaleString()}</td>
                                                        <td className="p-4 text-sm text-right text-emerald-400">{item.cash.toLocaleString()}</td>
                                                        <td className="p-4 text-sm text-right text-red-400">{item.credit.toLocaleString()}</td>
                                                        <td className="p-4 text-sm text-gray-500">{item.cashier}</td>
                                                        <td className="p-4 text-center">
                                                            <div className="flex items-center justify-center gap-1">
                                                                <button 
                                                                    onClick={() => handleViewPayments(item.vno)}
                                                ...
                                                {totals && (
                                                    <tr className="bg-gray-750 font-bold text-white border-t-2 border-gray-600">
                                                        ...
                                                        <td className="p-4 text-right text-blue-400">{totals.totalOther.toLocaleString()}</td>
                                                        ...
                                                        <td className="p-4 text-right text-emerald-400">{totals.totalCash.toLocaleString()}</td>
                                                        <td className="p-4 text-right text-red-400">{totals.totalCredit.toLocaleString()}</td>
                                                        <td colSpan={2}></td>
                                                    </tr>
                                                )}
                                            </>
                                            ...
                                            <tr>
                                                <td colSpan={sessionManager.getUserType() === 'admin' ? 17 : 16} className="p-8 text-center text-gray-500">
                                ...
                                <div className="overflow-y-auto flex-1 min-h-0">
                                    <Voucher
                                        voucher={{
                                            ...
                                            otherAmt: voucherDetails.voucher.otherAmt,
                                            ...
                                            paymentType: voucherDetails.voucher.paymentType || 'Credit',
            ...
            {isPayViewModalOpen && (
                <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                    <div className="bg-gray-800 rounded-2xl w-full max-w-2xl shadow-2xl border border-gray-700 overflow-hidden animate-in fade-in zoom-in duration-200 flex flex-col max-h-[90vh]">
                        <div className="p-4 bg-gray-900 border-b border-gray-700 flex justify-between items-center">
                            <h3 className="text-xl font-bold text-white flex items-center gap-2">
                                <CreditCard size={20} className="text-yellow-400" /> Payment Details
                            </h3>
                            <div className="flex items-center gap-2">
                                <button 
                                    onClick={() => mutatePayments()} 
                                ...
                                <button 
                                    onClick={() => { setIsPayViewModalOpen(false); setPaymentVNO(null); }} 
                        ...
                        {isLoadingPayments ? (
                            ...
                            <div className="p-6 overflow-y-auto">
                                {/* Voucher Summary */}
                                {paymentDetails.voucher && (
                                    <div className="bg-gray-900 rounded-lg p-4 mb-4 border border-gray-700">
                                        <div className="grid grid-cols-2 gap-4 text-sm">
                                            <div>
                                                <p className="text-gray-400">Folio No</p>
                                                <p className="text-white font-mono font-bold">{paymentDetails.voucher.vno}</p>
                                            </div>
                                            <div>
                                                <p className="text-gray-400">Customer</p>
                                                <p className="text-white">{paymentDetails.voucher.customerName || 'Walk-in'}</p>
                                            </div>
                                            <div>
                                                <p className="text-gray-400">Voucher Total</p>
                                                <p className="text-white font-bold">{paymentDetails.voucher.voucherTotal?.toLocaleString() || '0'} MMK</p>
                                            </div>
                                            <div>
                                                <p className="text-gray-400">Remaining Credit</p>
                                                <p className="text-orange-400 font-bold">{paymentDetails.voucher.remainingCredit?.toLocaleString() || '0'} MMK</p>
                                            </div>
                                            <div>
                                                <p className="text-gray-400">Total Paid</p>
                                                <p className="text-green-400 font-bold">{paymentDetails.totalPaid?.toLocaleString() || '0'} MMK</p>
                                            </div>
                                            <div>
                                                <p className="text-gray-400">Total Cash</p>
                                                <p className="text-blue-400 font-bold">{paymentDetails.voucher.totalCash?.toLocaleString() || '0'} MMK</p>
                                            </div>
                                        </div>
                                    </div>
                                )}
                                ...
                                {/* Payment List */}
                                <div>
                                    <h4 className="text-lg font-bold text-white mb-3">Payment History</h4>
                                            ...
                                            <table className="w-full text-left">
                                                <thead>
                                                    <tr className="bg-gray-800 border-b border-gray-700">
                                                        ...
                                                        <th className="p-3 text-xs font-bold text-gray-400 uppercase text-right">Amount</th>
                                                        <th className="p-3 text-xs font-bold text-gray-400 uppercase">User</th>
                                                    </tr>
                                                </thead>
                                                <tbody className="divide-y divide-gray-700">
                                                    {paymentDetails.payments.map((payment: any) => (
                                                        <tr key={payment.id} className="hover:bg-gray-750 transition-colors">
                                                            <td className="p-3 text-sm text-gray-300">
                                                                {formatDateTime(payment.date)}
                                                            </td>
                                                            <td className="p-3 text-sm text-right font-bold text-green-400">
                                                                {payment.amount?.toLocaleString() || '0'} MMK
                                                            </td>
                                                ...
                                                <tfoot>
                                                    <tr className="bg-gray-800 border-t-2 border-gray-600">
                                                        <td className="p-3 text-sm font-bold text-white">Total Paid</td>
                                                        <td className="p-3 text-sm text-right font-bold text-green-400">
                                                            {paymentDetails.totalPaid?.toLocaleString() || '0'} MMK
                                                        </td>
                                                        <td></td>
                                                    </tr>
                                                </tfoot>
