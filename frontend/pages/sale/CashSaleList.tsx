...
const CashSaleList: React.FC = () => {
  ...
  const { data, error, isLoading, mutate } = useSWR(
    `${API_ENDPOINTS.SALE_LIST_CASH}?${buildQueryString()}`,
    fetcher,
    SWR_CONFIG
  );
  ...
  const exportToExcel = () => {
    ...
    const title = `Cash Sale List (${appliedFilters.fromDate || 'All'} to ${appliedFilters.toDate || 'Today'})`;
    const isAdmin = sessionManager.getUserType() === 'admin';
    const headers = isAdmin 
        ? ["Date", "Folio No", "Branch", "Customer", "SubTotal", "Disc", "Tax", "Other", "Total", "Purchase Price", "Profit", "Cash", "Refund", "Cashier"]
        : ["Date", "Folio No", "Customer", "SubTotal", "Disc", "Tax", "Other", "Total", "Purchase Price", "Profit", "Cash", "Refund", "Cashier"];
    ...
    exportStyledExcel(title, headers, excelData, `cash_sales_${timestamp}.xlsx`, 'Cash Sales');
  };
      ...
      <header className="bg-gray-800 shadow-md p-4 flex items-center justify-between border-b border-gray-700 sticky top-0 z-40 shrink-0 h-16">
        <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
                ...
                <h1 className="text-xl font-bold font-myanmar">Cash အရောင်းစာရင်း</h1>
            </div>
        </div>
      </header>
                    ...
                    <div className="overflow-x-auto">
                        ...
                        <thead>
                            <tr className="bg-gray-900/50 text-gray-400 text-xs uppercase font-bold tracking-wider border-b border-gray-700">
                                ...
                                <th className="p-4 text-center min-w-[120px] whitespace-nowrap">Actions</th>
                            </tr>
                        </thead>
                        ...
                        <div className="overflow-y-auto flex-1 min-h-0">
                            <Voucher
                                voucher={{
                                    ...
                                    paymentType: voucherDetail.voucher.paymentType || 'Cash',
